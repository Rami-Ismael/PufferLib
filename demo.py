...
def train(args, env_module, make_env):
    ...
    if args.backend == 'clean_pufferl':
        ...
        while data.global_step < args.train.total_timesteps and data.losses.policy_loss <= 0.0 and (data.losses.value_loss == 0.00 or data.losses.value_loss >= 0.096) and (data.losses.old_approx_kl == 0.00 or data.losses.old_approx_kl >= 0.08):
            try:
